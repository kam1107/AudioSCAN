#!/usr/bin/perl
# prepare data (Public) for speaker verfication tasks

if (@ARGV != 2) {
  print STDERR "Usage: $0 <path-to-synthtrials> <path-to-data-dir>\n";
  print STDERR "e.g. $0 /export/synth_trials data/synth_trials\n";
  exit(1);
}
...
while (<TRIAL_IN>) {
  ...
  my $wav = "ffmpeg -v 8 -i $data_base/$path1 -f wav -acodec pcm_s16le -|";
  my ($meeting, $filename) = split('/', $path1);
  ...
  my $wav = "ffmpeg -v 8 -i $data_base/$path2 -f wav -acodec pcm_s16le -|";
  my ($meeting, $filename) = split('/', $path2);
  (my $segment = $filename) =~ s/\.[^.]+$//;
