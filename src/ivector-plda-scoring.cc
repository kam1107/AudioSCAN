...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    Output ofile("/data/greyostrich/not-backed-up/aims/aimsre/xxlu/audio_scan/kaldi/egs/voxceleb/v2/vectors/trials.txt",false);
    for (; !train_ivector_reader.Done(); train_ivector_reader.Next()) {
      ...
      transformed_ivector->Write(ofile.Stream(),false);
      num_train_ivectors++;
    }
