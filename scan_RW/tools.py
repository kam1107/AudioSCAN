...
def parse_wifi_file(path, threshold, interval):
    result = {}
    statistics = {}
...
def top_k_guesses_str(logits, ground_truth, class_names, num_chances):
    prediction_top_k = prob2decision(logits, num_chances)
    correct_prediction_counter = 0
    for truth, guesses in zip(ground_truth, prediction_top_k):
        guessed_names = [class_names[i] for i in guesses]
        if truth in guessed_names:
            correct_prediction_counter += 1
    refined_acc = np.array(correct_prediction_counter, dtype=np.float32) / len(ground_truth)
    return refined_acc
...
def scan_func(pic_features, pic_paths, wifi_people_in_meetings, peoples,
              video_info, meeting_index, meeting_num, hyper_para=1, frac_start=0.75):

    linkage_mat = linkage(pic_features, method='average')
    # generate cluster nodes
    rootnode, nodelist = to_tree(linkage_mat, rd=True)
    nb_leaves = len(leaves_list(linkage_mat))
    print('Total # of pics: {}'.format(nb_leaves))

    # decide the starting position of useful IDs to avoid tiny clusters
    start_node_idx = int(len(nodelist) * frac_start)

    # assign pic to useful nodes (clusters), avoiding meaningless association
    stat = {}
    cluster_feature = {}
    useful_nodelist = list()
    useful_id = 0
    for node in nodelist:
        ...
        node.pics = set([])
        stat[useful_id] = set([])
        cluster_feature[useful_id] = list()
        for leaf in node.pre_order():
            pic_path = pic_paths[leaf]
            cluster_feature[useful_id].append(video_info[leaf])
            parent = get_parent_folder_name(pic_path, 3)
            tem = parent.split('_')
            node.pics.add('%s_%s' % (tem[0], tem[1]))
    ...
    # assign event vector for each node and prepare pic path for res_stat
    pic_people_in_meetings = np.zeros([len(useful_nodelist), meeting_num])
    for node in useful_nodelist:
        for pic in node.pics:
            pic_people_in_meetings[node.useful_id, meeting_index[pic]] = 1
        for pic_idx in node.pre_order():
            pic_path = pic_paths[pic_idx]
            res_stat[node.useful_id][pic].append(pic_path)

    # print('pre_order # is {}, pic_idx is {}'.format(len(node.pre_order()), pic_idx))
    # print('pic_path is {}'.format(pic_path))
    # print('useful ID is {}, leaf_sample is {}, data is {}'.format(node.useful_id, node.pre_order(), res_stat[node.useful_id][pic]))
    # d = list(res_stat.values())[-5]

    # get the association cost matrix
    dist_mat = cdist(pic_people_in_meetings, wifi_people_in_meetings)
    print('distance matrix is {}'.format(dist_mat.shape))

    rows = [str(i) for i in range(dist_mat.shape[0])]
    cols = [str(i) for i in range(dist_mat.shape[1])]

    all_paths_to_leaves = parse(to_tree(linkage_mat), [], [])
    # init pulp
    prob = pulp.LpProblem("scan_opt", pulp.LpMinimize)
    assignment = pulp.LpVariable.dicts('assignment', (rows, cols), cat="Binary")
    # obj function
    prob += pulp.lpSum([assignment[r][c] * (dist_mat[int(r)][int(c)] + hyper_para * useful_nodelist[int(r)].dist)
                       for r in rows for c in cols])
    
    print('############### print dist to file')
    with open(join(os.getcwd(), 'final_result', '%s_%s.txt' % (hyper_para,frac_start)), 'w') as f:
        for r in rows:
            for c in cols:
                f.write(str(useful_nodelist[int(r)].dist))
                f.write(' ')
    f.close()

    

    print('# enforce constraints for useful nodes (clusters) and IDs respectively')
    for r in rows:
        prob += pulp.lpSum([assignment[r][c] for c in cols]) <= 1
    for c in cols:
        prob += pulp.lpSum([assignment[r][c] for r in rows]) == 1
    print('# enforce the hierachy relation constraints between nodes')
    ...
    for r in rows:
        for c in cols:
            if assignment[r][c].varValue > 0:
                ...
                print('Cluster {} is {}'.format(int(r)+start_node_idx, peoples[int(c)]))
                nb_labelled_images += len(useful_nodelist[int(r)].pre_order())

    print('Optimal value is {}'.format(pulp.value(prob.objective)))
    print('# of labelled images is {}'.format(nb_labelled_images))
    return final_res, save_matrix
